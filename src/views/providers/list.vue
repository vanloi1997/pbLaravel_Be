<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <router-link :to="'/providers/edit/'+scope.row.id">
            <el-button
              type="warning"
              size="mini"
              icon="el-icon-edit"
          ...
          <router-link :to="'/providers/delete/'+scope.row.id">
            <el-button
              type="danger"
              size="mini"
              icon="el-icon-delete"
...
export default class ProviderList extends Vue {
  ...
  private async getList() {
    ...
    const res = await new ProviderApi().index(this.listQuery)
    this.list = res.items
  ...
  private async handleDelete(e: any, id: number) {
    try {
      ...
      const state = await this.$confirm(
        'Confirm to remove this Provider?',
        'Warning',
        {
      ...
      if (state) {
        const res = await new ProviderApi().delete(id)
        if (res.affected > 0) {
          await this.getList()
          return this.$message({
            type: 'success',
      ...
      return this.$message({
        type: 'error',
        message: 'Some errors occured. Provider has not been Deleted!'
      })
    }
  }
}
