<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <router-link :to="'/sliders/edit/'+scope.row.id">
            <el-button
              type="warning"
              size="mini"
              icon="el-icon-edit"
          ...
          <router-link :to="'/sliders/delete/'+scope.row.id">
            <el-button
              type="danger"
              size="mini"
              icon="el-icon-delete"
...
export default class SliderList extends Vue {
  ...
  private async getList() {
    ...
    const res = await new SliderApi().index(this.listQuery)
    this.list = res.items
  ...
  private async handleDelete(e: any, id: number) {
    try {
      ...
      if (state) {
        const res = await new SliderApi().delete(id)
        if (res.affected > 0) {
          await this.getList()
          return this.$message({
            type: 'success',
      ...
      return this.$message({
        type: 'error',
        message: 'Some errors occured. Slider has not been Deleted!'
      })
    }
  }
}
