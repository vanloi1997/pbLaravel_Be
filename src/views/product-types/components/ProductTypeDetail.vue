          ...
          <el-col :span="24">
            <el-form-item style="margin-bottom: 40px;">
              <MDinput
                ...
                v-validate="`required|unique:product-types,${postForm.id}`"
                :maxlength="300"
                name="name"
                @blur="setSlug"
              >
...
export default class CategoryDetail extends Vue {
  ...
  private async fetchData(id: number) {
    const productType = await new ProductTypeApi().show(id)
    this.postForm = productType
    this.setTagsViewTitle()
    this.setPageTitle()
  }
  private setTagsViewTitle() {
    const title = this.lang === 'vi' ? 'Sửa sản phẩm' : 'Edit ProductType'
  ...
  private async submitForm() {
    ...
    const res = this.isEdit
      ? await new ProductTypeApi().update(this.postForm)
      : await new ProductTypeApi().create(this.postForm)
    this.loading = true
    this.$notify({
      title: 'Success',
      message: 'ProductType save successfully!',
      type: 'success',
      duration: 2000
    })
