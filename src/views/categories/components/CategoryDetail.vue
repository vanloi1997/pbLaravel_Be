          ...
          <el-col :span="24">
            <el-form-item style="margin-bottom: 40px;">
              <MDinput
                ...
                v-validate="`required|unique:categories,${postForm.id}`"
                :maxlength="300"
                name="name"
                @blur="setSlug"
              >
...
export default class CategoryDetail extends Vue {
  ...
  private async fetchData(id: number) {
    const category = await new CategoryApi().show(id)
    this.postForm = category
    this.setTagsViewTitle()
    this.setPageTitle()
  }
  private setTagsViewTitle() {
    const title = this.lang === 'vi' ? 'Sửa danh mục' : 'Edit Category'
  ...
  private async submitForm() {
    ...
    const res = this.isEdit
      ? await new CategoryApi().update(this.postForm)
      : await new CategoryApi().create(this.postForm)
    this.loading = true
    this.$notify({
      title: 'Success',
