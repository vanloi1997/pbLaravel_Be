          ...
          <el-col :span="24">
            <el-form-item style="margin-bottom: 40px;">
              <MDinput
                ...
                v-validate="`required|unique:bill-states,${postForm.id}`"
                :maxlength="300"
                name="name"
              >
                Name
...
export default class BillStateDetail extends Vue {
  ...
  private async fetchData(id: number) {
    const billState = await new BillStateApi().show(id)
    this.postForm = billState
    this.setTagsViewTitle()
    this.setPageTitle()
  }
  private setTagsViewTitle() {
    const title = this.lang === 'vi' ? 'Sửa danh mục' : 'Edit BillState'
  ...
  private async submitForm() {
    ...
    const res = this.isEdit
      ? await new BillStateApi().update(this.postForm)
      : await new BillStateApi().create(this.postForm)
    this.loading = true
    this.$notify({
      title: 'Success',
      message: 'BillState save successfully!',
      type: 'success',
      duration: 2000
    })
